...
function randomWord() {
  const randomIndex=Math.ceil(Math.random()*n);
  return words[randomIndex-1];
}

// different div for each word and inside it all letters in different span
function formatWord(word) {
  return `<div class="word"><span class="letter">${word.split('').join('</span><span class="letter">')}</span></div>`;
}

// adding to the className 
function addClass(ele,name) {
    ele.className += ' '+name;
}

// removing from the className
...
function newGame(){
  ...
  // first element that matches the css
  addClass(document.querySelector('.word'), 'current');
  addClass(document.querySelector('.letter'), 'current');
  // writing current class to first letter and first word
